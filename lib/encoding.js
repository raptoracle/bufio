...
function readVarintNum(data, off) {
  let value, size;
  ...
  switch (data[off]) {
    case 0xff:
      size = 9;
      checkRead(off + size <= data.length, off);
      value = readBigU64(data, off + 1);
      check(value > MAX_SAFE_INTEGER, off, 'Number too large to retain precision - use readVarintBN');
      break;
    case 0xfe:
      size = 5;
      checkRead(off + size <= data.length, off);
...
function readVarintBN(data, off) {
  let value, size;
  ...
  switch (data[off]) {
    ...
    case 0xfe:
      size = 5;
      checkRead(off + size <= data.length, off);
      value = new BN(readU32(data, off + 1));
      check(value.gt(0xffff), off, 'Non-canonical varint');
      break;
    case 0xfd:
      size = 3;
      checkRead(off + size <= data.length, off);
      value = new BN(readU16(data, off + 1));
      check(value.gte(0xfd), off, 'Non-canonical varint');
      break;
    default:
      size = 1;
      value = new BN(data[off]);
      break;
  }
...
function writeVarintBN(dst, num, off) {
  //enforce(Number.isSafeInteger(num), 'num', 'integer');
  num = new BN(num);

  if (num.lt(new BN(0xfd))) {
    dst[off++] = num.toNumber();
    return off;
  }

  if (num.lte(new BN(0xffff))) {
    dst[off++] = 0xfd;
    return writeU16(dst, num.toNumber(), off);
  }

  if (num.lte(new BN(0xffffffff))) {
    dst[off++] = 0xfe;
    return writeU32(dst, num.toNumber(), off);
  }

  dst[off++] = 0xff;

  return writeBigU64(dst, BigInt(num.toString(10)), off);
}

function sizeVarintBN(num) {
  num = new BN(num);
  if (num.lt(new BN(0xfd)))
    return 1;
...
exports.readVarintNum = readVarintNum;
exports.readVarintBN = ensureBigInt(readVarintBN);
exports.writeVarintBN = ensureBigInt(writeVarintBN);
exports.sizeVarintBN = ensureBigInt(sizeVarintBN);
