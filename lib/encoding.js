...
function readVarintNum(data, off) {
  ...
  switch (data[off]) {
    case 0xff:
      size = 9;
      checkRead(off + size <= data.length, off);
      value = readBigU64(data, off);
      check(value > MAX_SAFE_INTEGER, off, 'Number too large to retain precision - use readVarintBN');
      break;
    case 0xfe:
      size = 5;
      checkRead(off + size <= data.length, off);
      value = readU32(data, off);
      check(value > 0xffff, off, 'Non-canonical varint');
      break;
    case 0xfd:
      size = 3;
      checkRead(off + size <= data.length, off);
      value = readU16(data, off);
      check(value >= 0xfd, off, 'Non-canonical varint');
      break;
    default:
      size = 1;
      value = readU8(data, off);
      break;
  }
...
function readVarintBN(data, off) {
  ...
  switch (data[off]) {
    case 0xff:
      ...
      //check(value > Math.pow(2,53), off, 'Number too large to retain precision - use readVarintBN');
      break;
    case 0xfe:
      size = 5;
      checkRead(off + size <= data.length, off);
    ...
    default:
      size = 1;
      value = new BN(readU8(data, off + 1));
      break;
  }
...
exports.readVarintBN = readVarintBN;
exports.writeVarintBN = writeVarintBN;
